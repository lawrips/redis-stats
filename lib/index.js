                ...
                for (let status of statuses) {                
                    ...
                    for (let statusItem of status[1].split(',')) {
                        ...
                        if (this.stats.indexOf(statusKey) > -1) {
                            ...
                            // two cases:
                            // 1. value of '100' becomes {2016-09-10T10:00:00.000Z' : {val: '100'}}
                            // 2. value of '100M' becomes {unit: 'M', 2016-09-10T10:00:00.000Z' : val: '100'}
                            obj[time.toISOString()] = key.endsWith('_human') ? statusVal.slice(0,-1) : statusVal;
                            if (key.endsWith('_human')) {
                                obj.unit = statusVal.slice(-1);
                            }
                            ...
                            let hourTime = time.toISOString().slice(0,13) + ':00:00.000Z';
                            let unit, val;
                            if (key.endsWith('_human')) {
                                unit = statusVal.slice(-1);
                                val = Number.parseFloat(statusVal.slice(0,-1));
                            }
                            else {
                                val = Number.parseFloat(statusVal);                                
                            }
                            internals.addAvgToZsets(this.cluster || redis, key, new Date(hourTime).getTime(), hourTime, val, unit);                    
                            ...
                            internals.addAvgToZsets(this.cluster || redis, key, new Date(dayTime).getTime(), dayTime, val, unit);                    
                        }
                    };
                }
            }); 
...
internals.addAvgToZsets = function(redisServer, key, score, objKey, objVal, unit) {
    redisServer.zrangebyscore(key, score, score, (err, result) => {
        ...
        if (!result || result.length == 0) {
            ...
            let n = result.n;
            let scale = {'B': 1, 'K': 1000, 'M': 1000000, 'G': 1000000000, 'T': 1000000000000};
            let newVal;
            if (unit) {
                newVal = ((n * scale[result.unit] * result[objKey]) + (scale[unit] * objVal)) / ((n+1) * scale[unit]);
            }
            else {
                newVal = (n * result[objKey] + objVal) / (n+1);
            }
            obj[objKey] = newVal;
            if (unit) {
                obj.unit = unit;
